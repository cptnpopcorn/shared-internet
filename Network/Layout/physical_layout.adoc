...
* A PoE injector powering a relay board (extracteur control), static IP `10.137.0.50`, on the pre-wired port 15 (VLAN 470 access)
...
* A Raspberry Pi 4 to provide user services (collaboration, organization, documentation), with an internal DNS entry 'services.eco'
* A PoE injector powering a relay board (extracteur control), static IP `10.137.0.30`, on the pre-wired port 15 (VLAN 470 access)
...
* A PoE injector that powers a relay board (extracteur control), with the static IP `10.137.0.10`, connected to the building's DGS-1210-16 on *port 15* — an access port of VLAN 470 (IoT / relays, PVID 470)

[NOTE]
====
The relay VLAN (470, `10.137.0.0/16`) is routed by the Mikrotik in building A, so
a relay's frames travel tagged across the trunks:
building B DGS-1210 (port 20) -> DGS-3130 stack (eth7/0/24 … eth4/0/24) ->
building A DGS-1210 (ports 20 and 18) -> Mikrotik `sfp-sfpplus10`.
On *every* building's DGS-1210, *port 15* is pre-wired as a VLAN 470 access port
(untagged, PVID 470), so a relay board can be dropped into any building without
further switch configuration. A relay board is deployed in each of the three
buildings.
====
